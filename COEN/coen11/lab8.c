...
int main(int argc, char *argv[])
{
		...
		printf("6: Show each name backwards\n");
			...
			for (int q = 0; q < 4; q++)
			{
				...
				struct node *ptr = arr[q].head; //sets the pointer to the head of the index of the array
				back_show(ptr);
			}
			break;
			...
			for (int e = 0; e < 4; e++)
			{
				struct node *p = arr[e].head; // sets the pointer to the head of the index of the array
				if (e == 0) // prints each thing depending on e
					printf("Sizes 1 or 2\n");
				if (e == 1)
                                        printf("Sizes 3 or 4\n");
				if (e == 2)
...
void back_name(struct node *p)
{
	...
	for (int i = strlen(p->name) - 1; i >= 0; i--) //starts from the last character in the string so that it prints it backwards
	{
		printf("%c", p->name[i]);
	}
	printf("\n");
